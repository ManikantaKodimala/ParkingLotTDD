...
public class DisplayTruckSlots implements Display {

    private  static int slotStartsAt=1;
    private static int slotEndsAt=1;
    ...
    public void displayOccupiedSlots(ParkingLot parkingLot) {
        ...
        for (int i = 1; i <= parkingLot.getNoOfFloors(); i++) {
            if (occupiedFloors.containsKey(i)) {
                ...
                System.out.println("Occupied slots for CAR on Floor" + i + " : " + freeSlots.toString());
                freeSlots.clear();
            }
        }
    }
